...
class pager {
    ...
    public function setRecPerPage($recPerPage) {
        ...
        if (isset($_COOKIE[$this->pageCookie]))
            $recPerPage = $_COOKIE[$this->pageCookie];
    ...
    private function createFirstPage() {
        ...
        return html::a(helper::createLink($this->moduleName, $this->methodName, $this->params) . $this->search_params, $this->lang->pager->first);
    }
    ...
    private function createPrePage($type = 'full') {
        if ($type == 'mobile') {
            ...
            $this->params['pageID'] = $this->pageID - 1;
            return html::a(helper::createLink($this->moduleName, $this->methodName, $this->params) . $this->search_params, $this->lang->pager->pre);
        }
    }
    ...
    private function createNextPage($type = 'full') {
        if ($type == 'mobile') {
            ...
            $this->params['pageID'] = $this->pageID + 1;
            return html::a(helper::createLink($this->moduleName, $this->methodName, $this->params) . $this->search_params, $this->lang->pager->next);
        }
    }
    ...
    public function getSearchParam() {
        ...
        foreach ($querys as $query) {
            if (!preg_match('/search_reset/i', $query)) {
                $search_param[] = $query;
            }
        }
    ...
    private function createLastPage() {
        ...
        return html::a(helper::createLink($this->moduleName, $this->methodName, $this->params) . $this->search_params, $this->lang->pager->last);
    }
    ...
    private function createRecPerPageJS() {
        ...
        foreach ($params as $key => $value) {
            if (strtolower($key) == 'rectotal')
                $params[$key] = '_recTotal_';
            if (strtolower($key) == 'recperpage')
                $params[$key] = '_recPerPage_';
            if (strtolower($key) == 'pageid')
                $params[$key] = '_pageID_';
        }
        $vars = '';
        ...
        {
            pageTotal  = parseInt($('#_pageTotal').val());
            pageID     = $('#_pageID').val();
            recPerPage = $('#_recPerPage').val();
            recTotal   = $('#_recTotal').val();
            ...
            location.href= createLink('$this->moduleName', '$this->methodName', vars) + '$this->search_params';
        }
        </script>
EOT;
        return $js;
