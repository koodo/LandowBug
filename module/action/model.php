...
class actionModel extends model
{
     ...
     * @return int
     */
    public function create($objectType, $objectID, $actionType, $comment = '', $extra = '', $actor = '', $assignTo = '')
    {
        $action = new stdclass();
        ...
        $action->actor      = $actor ? $actor : $this->app->user->realname;
    ...
    public function printAction($action)
    {
        ...
        $action->date = common::tTimeFormat(strtotime($action->date));
    ...
    public function transformActions($actions)
    {
        ...
        foreach($actions as $action)
        {
            ...
            // 操作时刻 modified by chenyongcai
            $action->date        = $this->tTimeFormat(strtotime($action->date));
    ...
    public function printChanges($objectType, $histories)
    {
        ...
        foreach($histories as $history)
        {
            ...
            {
                if($history->fieldLabel == '严重程度') {
                    $history->old = common::bug_serverity($history->old);
                    $history->new = common::bug_serverity($history->new);
                }
                if($history->fieldLabel == '指派给') {
    ...
    /**
     * Converse the unix time to recent string
     * 
     * @author chenyongcai
     * @param type $time
     * @access public 
     * @return void
     */
    public function timeConverse($stringtime){
        return $this->tTimeFormat(strtotime($stringtime));
    }
     ...
     * @return string
     */
    private function tTimeFormat($timestamp) {
        $curTime = time();
