...
class bugModel extends model {
    ...
    public function create() {
        ...
        $bug = fixer::input('post')
                ...
                ->join('openedBuild', ',')
                ->join('mailto', ',')
                ->remove('files, labels,copyid,copyFiles')
                ->get();
        ...
        if (!dao::isError()) {
            ...
            if ($this->post->copyFiles == "1") {
                // 复制附件
                $nbugid = $this->post->copyid;
                $SQL = "INSERT INTO zt_file (pathname,title,extension,size,objectType,objectID,addedBy,addedDate,downloads,extra,deleted) 
                    select pathname,title,extension,size,objectType,$bugID,addedBy,CURRENT_DATE(),0,'','0' from zt_file where objectID = $nbugid;";
                $this->dbh->query($SQL);
            }
            return $bugID;
        }
    ...
    public function batchCreate($productID) {
        ...
        for ($i = 0; $i < $this->config->bug->batchCreate; $i++) {
            ...
            if (dao::isError())
                die(js::error('bug#' . ($i + 1) . dao::getError(true)));
    ...
    public function getModuleBugs($productID, $moduleIds = 0, $projects, $orderBy = 'id_desc', $pager = null) {
        return $this->dao->select('*')->from(TABLE_BUG)
                        ->where('project')->eq((int) $productID)
     ...
     * @return void
     */
    public function getProjectById($projectID, $setImgSize = false) {
    ...
    public function getById($bugID, $setImgSize = false) {
        ...
        foreach ($bug as $key => $value)
            if (strpos($key, 'Date') !== false and !(int) substr($value, 0, 4))
                $bug->$key = '';

        if ($bug->duplicateBug)
            $bug->duplicateBugTitle = $this->dao->findById($bug->duplicateBug)->from(TABLE_BUG)->fields('title')->fetch('title');
        if ($bug->case)
            $bug->caseTitle = $this->dao->findById($bug->case)->from(TABLE_CASE)->fields('title')->fetch('title');
        if ($bug->linkBug)
            $bug->linkBugTitles = $this->dao->select('id,title')->from(TABLE_BUG)->where('id')->in($bug->linkBug)->fetchPairs();
        if ($bug->toStory > 0)
            $bug->toStoryTitle = $this->dao->findById($bug->toStory)->from(TABLE_STORY)->fields('title')->fetch('title');
        if ($bug->toTask > 0)
            $bug->toTaskTitle = $this->dao->findById($bug->toTask)->from(TABLE_TASK)->fields('name')->fetch('name');

        $bug->toCases = array();
        $toCases = $this->dao->select('id, title')->from(TABLE_CASE)->where('`fromBug`')->eq($bugID)->fetchAll();
        foreach ($toCases as $toCase)
            $bug->toCases[$toCase->id] = $toCase->title;
    ...
    public function update($bugID) {
        ...
        $bug = fixer::input('post')
                ...
                ->specialChars('title,keyword')
                ->remove('comment,files,labels,reassign_comment')
    ...
    /**
     * recheck a bug =>> active
     * @param type $bugID
     */
    public function recheck_bug($bugID) {
        $now = helper::now();
        $oldBug = $this->getById($bugID);
        $bug = fixer::input('post')
                ->setDefault('confirmed', 0)
     ...
     * @param type $bugID
     */
    public function suspend_bug($bugID) {
        $now = helper::now();

        $bug = fixer::input('post')
                ->setDefault('confirmed', 0)
                ->setDefault('status', 'suspend')
                ->setDefault('lastEditedBy', $this->app->user->account)
                ->setDefault('lastEditedDate', $now)
                ->remove('comment')
                ->join('mailto', ',')
    ...
    public function extractAccountsFromList($bugs) {
        $accounts = array();
        foreach ($bugs as $bug) {
            if (!empty($bug->openedBy))
                $accounts[] = $bug->openedBy;
            if (!empty($bug->assignedTo))
                $accounts[] = $bug->assignedTo;
            if (!empty($bug->resolvedBy))
                $accounts[] = $bug->resolvedBy;
            if (!empty($bug->closedBy))
                $accounts[] = $bug->closedBy;
            if (!empty($bug->lastEditedBy))
                $accounts[] = $bug->lastEditedBy;
        }
    ...
    public function getDataOfBugsPerProject() {
        $datas = $this->dao->select('project as name, count(project) as value')->from(TABLE_BUG)
                        ->where("deleted")->eq(0)
                        ->beginIF($this->post->time_from == $this->post->time_to)->andWhere('DATE(openedDate)')->eq($this->post->time_to)->fi()
                        ->beginIF($this->post->time_from != $this->post->time_to)->andWhere('DATE(openedDate)')->between($this->post->time_from, $this->post->time_to)->fi()
                        ->groupBy('project')->orderBy('value DESC')->fetchAll('name');
        if (!$datas)
            return array();
        $projects = $this->loadModel('project')->getPairs('nocode');
        foreach ($datas as $projectID => $data) {
            if (isset($projects[$projectID]))
                $data->name = $projects[$projectID];
            else
    ...
    public function getDataOfBugsPerBuild() {
        $datas = $this->dao->select('openedBuild as name, count(openedBuild) as value')->from(TABLE_BUG)
                        #->where('severity')->ne(0)
                        ->where('project')->eq($this->cookie->lastProject)
                        ->andwhere('deleted')->eq(0)
                        ->beginIF($this->post->time_from == $this->post->time_to)->andWhere('DATE(openedDate)')->eq($this->post->time_to)->fi()
                        ->beginIF($this->post->time_from != $this->post->time_to)->andWhere('DATE(openedDate)')->between($this->post->time_from, $this->post->time_to)->fi()
                        ->groupBy('openedBuild')->orderBy('value DESC')->fetchAll('name');
        if (!$datas)
            return array();
        $builds = $this->loadModel('build')->getProjectBuildPairs($this->cookie->lastProject);
    ...
    public function getDataOfBugsPerModule() {
        ...
        if (!$datas)
            return array();
        $modules = $this->loadModel('module')->getPairs($this->cookie->lastProject);
    ...
    public function getDataOfOpenedBugsPerUser() {
        ...
        if (!$datas)
            return array();
        if (!isset($this->users))
            $this->users = $this->loadModel('user')->getPairs('noletter');
    ...
    public function getDataOfBugsPerSeverity() {
        ...
        if (!$datas)
            return array();
        foreach ($datas as $severity => $data)
            if (isset($this->lang->bug->severityList[$severity]))
                $data->name = $this->lang->bug->severityList[$severity];
        return $datas;
    }
    ...
    public function getDataOfBugsPerResolution() {
        ...
        if (!$datas)
            return array();
        foreach ($datas as $resolution => $data)
            if (isset($this->lang->bug->resolutionList[$resolution]))
                $data->name = $this->lang->bug->resolutionList[$resolution];
        return $datas;
    }
    ...
    public function getDataOfBugsPerStatus() {
        ...
        foreach ($datas as $status => $data) {
            $data->name = $this->lang->bug->_statusList[$status];
        }
        return $datas;
    }
    ...
    public function getDataOfBugsPerType() {
        ...
        foreach ($datas as $type => $data)
            if (isset($this->lang->bug->typeList[$type]))
                $data->name = $this->lang->bug->typeList[$type];
        return $datas;
    }
    ...
    public function mergeChartOption($chartType) {
        ...
        if (!isset($chartOption->swf))
            $chartOption->swf = $commonOption->swf;
        if (!isset($chartOption->width))
            $chartOption->width = $commonOption->width;
        if (!isset($chartOption->height))
            $chartOption->height = $commonOption->height;
     ...
     * @return array
     */
    public function getUrgentBugs($productID, $projects, $orderBy = 'severity_desc', $pager = null, $search = false) {
        if (!$search) {
            $bugs = $this->dao->select('t1.*')
                            ->from(TABLE_BUG)->alias('t1')
                            ->where('t1.project')->eq($productID)
                            ->andWhere('t1.deleted')->eq(0)
                            ->andWhere('t1.severity')->gt(2)
                            ->andWhere('t1.status')->eq('active')
                            ->andWhere('DATEDIFF(NOW(),t1.openedDate)')->lt(30)
                            ->orderBy($orderBy)->page($pager)->fetchAll();
        } else {
            $bugs = $this->dao->select('t1.*')
                            ->from(TABLE_BUG)->alias('t1')
                            ...
                            ->andWhere('t1.status')->eq('active')
                            ->andWhere('DATEDIFF(NOW(),t1.openedDate)')->lt(30)
                            ->beginIF($_GET['search_status'] != '#')->andWhere('t1.status')->eq($_GET['search_status'])->fi()
                            ->beginIF($_GET['search_severity'] != '#')->andWhere('t1.severity')->eq($_GET['search_severity'])->fi()
                            ->andWhere('t1.title')->like("%" . $_GET['search_key'] . "%")
                            ->orderBy($orderBy)->page($pager)->fetchAll();
        }
        return $bugs;
    }

    public function getNewsetBugs($productID, $projects, $orderBy = 't1.openedDate_desc', $pager = null, $search = false) {
        // 获取最近的bug，取30天以内
        if (!$search) {
            $bugs = $this->dao->select('t1.*')
                            ->from(TABLE_BUG)->alias('t1')
     ...
     * @return array
     */
    public function getAllBugs($productID, $projects, $orderBy = 't1.openedDate_desc', $pager = null, $search = false) {
        // 获取最近的bug，取30天以内
        if (!$search) {
            $bugs = $this->dao->select('t1.*')
                            ->from(TABLE_BUG)->alias('t1')
            ...
            $bugs = $this->dao->select('t1.*')
                            ...
                            ->andWhere('DATEDIFF(NOW(),t1.openedDate)')->lt(30)
                            ->beginIF($_GET['search_status'] != '#')->andWhere('t1.status')->eq($_GET['search_status'])->fi()
                            ->beginIF($_GET['search_module'] != '#')->andWhere('t1.module')->eq($_GET['search_module'])->fi()
                            ->beginIF($_GET['search_openedby'] != '#')->andWhere('t1.openedBy')->eq($_GET['search_openedby'])->fi()
                            ->beginIF($_GET['search_assignto'] != '#')->andWhere('t1.assignedTo')->eq($_GET['search_assignto'])->fi()
                            ->beginIF($_GET['search_severity'] != '#')->andWhere('t1.severity')->eq($_GET['search_severity'])->fi()
                            ->andWhere('t1.title')->like("%" . $_GET['search_key'] . "%")
                            ->orderBy($orderBy)->page($pager)->fetchAll();
        }
     ...
     * @return type
     */
    public function getAllBugsCount($productID, $projects) {
        $bugs = $this->dao->select('COUNT(*) AS count')
                        ->from(TABLE_BUG)->alias('t1')
                        ->where('t1.project')->eq($productID)->fetchAll();
        return $bugs[0];
    }
     ...
     * @return type
     */
    public function getUnresolvedBugsCount($productID, $projects) {
        $bugs = $this->dao->select('COUNT(*) AS count')
                        ->from(TABLE_BUG)->alias('t1')
                        ->leftJoin(TABLE_PRODUCTPLAN)->alias('t2')->on('t1.plan = t2.id')
                        ->where('t1.project')->eq($productID)
                        ->andWhere('t1.deleted')->eq(0)
                        ->andWhere('t1.status')->eq('active')->fetchAll();
        return $bugs[0];
    }
     ...
     * @return type
     */
    public function getUnresolvedBugs($productID, $projects, $orderBy, $pager, $search = false) {
        if (!$search) {
     ...
     * @return type
     */
    public function getNotConFirmedBugsCount($productID, $projects) {
        $bugs = $this->dao->select('COUNT(*) AS count')
                        ->from(TABLE_BUG)->alias('t1')
                        ->leftJoin(TABLE_PRODUCTPLAN)->alias('t2')->on('t1.plan = t2.id')
                        ->where('t1.project')->eq($productID)
                        ->andWhere('t1.deleted')->eq(0)
                        ->andWhere('t1.status')->eq('needcheck')->fetchAll();
        return $bugs[0];
    }
     ...
     * @return type
     */
    public function getBuildBugCount($build, $project) {
        $bugs = $this->dao->select('COUNT(*) AS count')
                        ->from(TABLE_BUG)->alias('t1')
                        ->where('t1.project')->eq($project)
                        ->andwhere('t1.openedBuild')->eq($build)
                        ->andWhere('t1.deleted')->eq(0)->fetchAll();
        return $bugs[0];
    }
     ...
     * @return type
     */
    public function getSuspendCount($build, $project) {
        $bugs = $this->dao->select('COUNT(*) AS count')
                        ->from(TABLE_BUG)->alias('t1')
                        ->where('t1.project')->eq($project)
                        ->andwhere('t1.openedBuild')->eq($build)
                        ->andWhere('t1.deleted')->eq(0)
                        ->andWhere('t1.status')->eq('suspend')->fetchAll();
        return $bugs[0];
    }
     ...
     * @return type
     */
    public function getByClosed($productID, $projects, $orderBy, $pager, $search = false) {
        if (!$search) {
            $bugs = $this->dao->select('t1.*')
                            ->from(TABLE_BUG)->alias('t1')
                            ->where('t1.project')->eq($productID)
     ...
     * @return type
     */
    public function getByClosedCount($productID, $projects) {
        $bugs = $this->dao->select('COUNT(*) AS count')
                        ->from(TABLE_BUG)->alias('t1')
                        ->leftJoin(TABLE_PRODUCTPLAN)->alias('t2')->on('t1.plan = t2.id')
                        ->where('t1.project')->eq($productID)
                        ->andWhere('t1.deleted')->eq(0)
                        ->andWhere('t1.status')->eq('closed')->fetchAll();
        return $bugs[0];
    }
     ...
     * @return type
     */
    public function getMySuspendCount($productID, $projects) {
        $bugs = $this->dao->select('COUNT(*) AS count')
                ->from(TABLE_BUG)->alias('t1')
                ->leftJoin(TABLE_PRODUCTPLAN)->alias('t2')->on('t1.plan = t2.id')
                ->where('t1.project')->eq($productID)
     ...
     * @return type
     */
    public function getAssigntoMeCount($productID, $projects) {
        $bugs = $this->dao->select('COUNT(*) AS count')
                ->from(TABLE_BUG)->alias('t1')
                ->leftJoin(TABLE_PRODUCTPLAN)->alias('t2')->on('t1.plan = t2.id')
                ->where('t1.project')->eq($productID)
                ->andWhere('t1.deleted')->eq(0)
                ->andWhere('t1.assignedTo')->eq($this->app->user->account)
                ->andWhere('t1.status')->in(array('active', 'suspend', 'needcheck'))
                ->fetchAll();
        return $bugs[0];
    }
     ...
     * @return type
     */
    public function getResolvedBugsCount($productID, $projects) {
        $bugs = $this->dao->select('COUNT(*) AS count')
                        ->from(TABLE_BUG)->alias('t1')
                        ->leftJoin(TABLE_PRODUCTPLAN)->alias('t2')->on('t1.plan = t2.id')
                        ->where('t1.project')->eq($productID)
                        ->andWhere('t1.deleted')->eq(0)
                        ->andWhere('t1.status')->eq('resolved')->fetchAll();
        return $bugs[0];
    }
    ...
    /**
     * Get bugs of assign to me that unfix. 
     * 
     * @param  int    $productID 
     * @param  array  $projects 
     * @param  int    $queryID 
     ...
     * @return array
     */
    public function getByAssigntomeUnfix($productID, $projects, $orderBy, $pager, $search = false) {
        // 未解决(active)|待审核(needcheck)|已挂起(suspend) 都属于未解决的bug
        if (!$search) {
            $bugs = $this->dao->select('t1.*')
                            ->from(TABLE_BUG)->alias('t1')
                            ->where('t1.project')->eq($productID)
    ...
    /**
     * Get bugs of assign to me that fixed. 
     * 
     * @param  int    $productID 
     * @param  array  $projects 
     * @param  int    $queryID 
     ...
     * @return array
     */
    public function getByAssigntomeFixedCount($productID, $projects, $orderBy, $search = false) {
        if (!$search) {
            $bugs = $this->dao->select('COUNT(t1.id) AS count')
                            ->from(TABLE_BUG)->alias('t1')
                            ->where('t1.project')->eq($productID)
                            ->andWhere('t1.deleted')->eq(0)
                            ->andWhere('t1.status')->eq('resolved')
                            ->andWhere('t1.resolvedBy')->eq($this->app->user->account)
    ...
    public function getByAssigntomeUnfixCount($productID, $projects, $orderBy, $search = false) {
        if (!$search) {
            $bugs = $this->dao->select('COUNT(*) AS count')
                            ->from(TABLE_BUG)->alias('t1')
                            ->where('t1.project')->eq($productID)
    ...
    public function getByAssigntomeFixed($productID, $projects, $orderBy, $pager, $search = false) {
        if (!$search) {
            $bugs = $this->dao->select('t1.*')
                            ->from(TABLE_BUG)->alias('t1')
                            ->where('t1.project')->eq($productID)
     ...
     * @return type
     */
    public function getByResolved($productID, $projects, $orderBy, $pager, $search = false) {
        if (!$search) {
            $bugs = $this->dao->select('t1.*')
                            ->from(TABLE_BUG)->alias('t1')
                            ->where('t1.project')->eq($productID)
     ...
     * @return type
     */
    public function getNotConfirmed($productID, $projects, $orderBy, $pager, $search = false) {
        if (!$search) {
            $bugs = $this->dao->select('t1.*')
                            ->from(TABLE_BUG)->alias('t1')
                            ->where('t1.project')->eq($productID)
                            ->andWhere('t1.deleted')->eq(0)
                            ->andWhere('t1.status')->eq('needcheck')
                            ->orderBy($orderBy)->page($pager)->fetchAll();
        } else {
            $bugs = $this->dao->select('t1.*')
                            ->from(TABLE_BUG)->alias('t1')
     ...
     * @return array
     */
    public function getByOpenedbyme($productID, $projects, $orderBy, $pager, $search = false) {
        if (!$search) {
            return $this->dao->findByOpenedBy($this->app->user->account)->from(TABLE_BUG)
                            ->andwhere('project')->eq($productID)
                            ->andWhere('deleted')->eq(0)
                            ->orderBy($orderBy)->page($pager)->fetchAll();
        } else {
            $bugs = $this->dao->select('t1.*')
    ...
    public function formCustomedBugs($bugs) {
        ...
        foreach ($bugs as $bug) {
            ...
            if (isset($relatedProjects[$bug->project]))
                $bug->project = $relatedProjects[$bug->project];
        }
        return $bugs;
    }
    ...
    public static function isClickable($object, $action) {
        ...
        if ($action == 'confirmbug')
            return $object->status == 'needcheck' and $object->confirmed == 0;
        if ($action == 'resolve')
            return $object->status == 'active';
        if ($action == 'activate')
            return $object->status == 'closed' || $object->status == 'suspend';
        if ($action == 'tostory')
            return $object->status == 'active';
        if ($action == 'assignto')
            return $object->status == 'active';
        if ($action == 'suspend')
            return $object->status == 'active';
        if ($action == 'recheck')
            return $object->status == 'needcheck';
        if ($action == 'close')
            return $object->status != 'closed' && $object->status != 'resolved';
        if ($action == 'edit')
