...
class projectModel extends model {
    ...
    public function checkPriv($project) {
        ...
        static $teams;
        if (empty($teams))
            $teams = $this->dao->select('project, account')->from(TABLE_TEAM)->fetchGroup('project', 'account');
        ...
        if ($project->acl == 'custom') {
            if (isset($currentTeam[$this->app->user->account]))
                return true;
            $userGroups = $this->app->user->groups;
            $projectGroups = explode(',', $project->whitelist);
            foreach ($userGroups as $groupID) {
                if (in_array($groupID, $projectGroups))
                    return true;
            }
            return false;
        }
    ...
    public function tree() {
        ...
        foreach ($productGroup as $productID => $projects) {
            if (!isset($products[$productID]) and $productID != '')
                continue;
            if (!isset($products[$productID]) and !count($projects))
                continue;
    ...
    public function saveState($projectID, $projects) {
        if ($projectID > 0)
            $this->session->set('project', (int) $projectID);
        if ($projectID == 0 and $this->cookie->lastProject)
            $this->session->set('project', (int) $this->cookie->lastProject);
        if ($projectID == 0 and $this->session->project == '')
            $this->session->set('project', $projects[0]);
        if (!in_array($this->session->project, $projects))
            $this->session->set('project', $projects[0]);
    ...
    public function create($copyProjectID = '') {
        ...
        $project = fixer::input('post')
                ->add('teamid', $this->post->pteam)
                ->add('openedBy', $app->user->account)
                ...
                ->join('whitelist', ',')
                ->remove('products, workDays, delta, days, roles, realname, accounts, hours, realnames, accounts, modes, pteam, isadmin')
                ->get();
        ...
        if (!dao::isError()) {
            ...
            if ($copyProjectID != '') {
                ...
                foreach ($members as $member) {
                    ...
                    if ($member->account == $this->app->user->account)
                        $creatorExists = true;
                }
            }
    ...
    public function update($projectID) {
        ...
        $project = fixer::input('post')
                ...
                ->setDefault('team', $this->post->name)
                ->join('whitelist', ',')
                ->remove('products, workDays, delta, days, roles, realname, accounts, hours, realnames, accounts, modes, isadmin,pteam')
                ->get();
    ...
    public function batchUpdate() {
        ...
        foreach ($projects as $projectID => $project) {
            ...
            if (dao::isError())
                die(js::error('project#' . $projectID . dao::getError(true)));
    ...
    public function getList($status = 'all', $limit = 0, $productID = 0) {
        if ($productID != 0) {
            ...
            return $this->dao->select('*, IF(INSTR(" done", status) < 2, 0, 1) AS isDone')->from(TABLE_PROJECT)->alias('t1')
                            ->leftJoin(TABLE_TEAMLIST)->alias('t2')
                            ->on('t1.teamid=t2.tid')
                            ->where('iscat')->eq(0)
    ...
    public function getProjectPairs() {
        ...
        foreach ($projects as $key => $project) {
            if (!$this->memberInProject($app->user->account, $project->id)) {
                unset($projects[$key]);
                continue;
            }
    ...
    public function getValidProjectId($ProjectId = 0) {
        global $app;
        $return = $ProjectId;
        $projects = $this->dao->select('t1.*')
                ...
                ->where('t1.deleted')->eq(0)
                ->andwhere('t2.account')->eq($app->user->account)
                ->andwhere('t1.id>=0')
                ->orderBy('t1.id desc')
                ->fetchAll();
        foreach ($projects as $key => $project) {
    ...
    public function getProjectStatsPair() {
        $projects = $this->getProjectStats();
        $datas = array();
        foreach ($projects as $project) {
            $datas[$project->id] = $project->name;
        }
        return $datas;
    }
    ...
    public function getProjectStats($status = 'undone', $productID = 0, $itemCounts = 30) {
        ...
        foreach ($projects as $key => $project) {
            ...
            if (!$this->memberInProject($app->user->account, $project->id) && $app->user->account != $project->openedBy) {
                unset($projects[$key]);
                continue;
            }
    ...
    public function getMemberProject() {
        ...
        $projects = $this->getList('all', 0, 0);
        foreach ($projects as $key => $project) {
            if (!$this->memberInProject($app->user->account, $project->id) && $project->openedBy != $app->user->account) {
                unset($projects[$key]);
                continue;
            }
        }
    ...
    public function isProjectAdmin($account, $projectId) {
        $data = $this->dbh->query(sprintf("SELECT COUNT(*) AS count FROM `zt_team` WHERE `account` = '%s' AND `project` = '%s' AND `isadmin` = 1;", $account, $projectId));
        foreach ($data as $count) {
            return (bool) $count->count;
        }
    }

    public function memberInProject($uid, $projectId) {
        $data = $this->dbh->query(sprintf("SELECT COUNT(*) AS count FROM `zt_team` WHERE `account` = '%s' AND `project` = '%s';", $uid, $projectId));
        foreach ($data as $count) {
            return (bool) $count->count;
        }
    }
    ...
    public function getProjectList($projectID) {
        $project = $this->dao->select('t1.*,t2.tname')
                ->from(TABLE_PROJECT)->alias('t1')
                ->leftJoin(TABLE_TEAMLIST)->alias('t2')
                ->on('t1.teamid = t2.tid')
                ->andWhere('t1.deleted')->eq(0)
    ...
    public function getProjectsToImport() {
        ...
        foreach ($projects as $id => $project) {
            if ($this->checkPriv($project) and ($project->status == 'done' or $project->end < $now))
                $pairs[$id] = ucfirst(substr($project->code, 0, 1)) . ':' . $project->name;
        }
        return $pairs;
    }
    ...
    public function getTeamMembers($projectID) {
        return $this->dao->select('t1.*, t2.realname, t2.id as uid')->from(TABLE_TEAM)->alias('t1')
    ...
    public function getTeamMemberPairs($projectID, $params = '') {
        ...
        foreach ($users as $account => $realName) {
            $users[$account] = ($realName ? $realName : $account);
        }
        return $users;
    }
     ...
     * @return void
     */
    public function manageMembers($projectID, $create = false) {
        extract($_POST);
        global $app;
        $accounts = array_unique($accounts);
        $account_admin = $app->user->account;
        $this->dbh->query("DELETE FROM `zt_team` WHERE `project` = '$projectID';");
        if ($create)
            $this->dbh->query("REPLACE INTO `zt_team` (`project`,`account`,`join`,`isadmin`) VALUES('$projectID','$account_admin',CURRENT_DATE(),'1')");
        ...
        foreach ($accounts as $key => $account) {
            ...
            $member = new stdclass();
            #$member->accounts = $accounts[$key];
            #$member->realnames = $realnames[$key];
            ...
            if ($mode == 'update') {
                ...
                $this->dbh->query("REPLACE INTO `zt_team`(`project`,`account`,`join`,`isadmin`) VALUES('$projectID','$accounts[$key]',CURRENT_DATE(),'$isadmin[$key]')");
            } else {
                $member->project = (int) $projectID;
    ...
    public static function isClickable($project, $action) {
        ...
        if ($action == 'suspend')
            return $project->status == 'wait' or $project->status == 'doing';
        if ($action == 'putoff')
            return $project->status == 'wait' or $project->status == 'doing';
        if ($action == 'activate')
            return $project->status == 'suspended' or $project->status == 'done';
    ...
    public function getProjectLink($module, $method, $extra) {
        ...
        if ($extra[0] == '_') {
            ...
            $link = helper::createLink($module, $method, "projectID=%s&browseType=$extra");
        } else {
    ...
    public function isProjectCreator($project) {
        global $app;
        $check = $this->dbh->query("SELECT COUNT(*) AS count FROM zt_project WHERE id = '$project' AND openedBy = '" . $app->user->account . "';");
        foreach ($check as $c) {
            if ($c->count == 1)
                return true;
            return false;
