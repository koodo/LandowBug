...
class userModel extends model {
    ...
    public function getPairs($params = '', $usersToAppended = '') {
        ...
        if (strpos($params, 'pofirst') !== false)
            $fields .= ", INSTR(',pd,po,', role) AS roleOrder";
        if (strpos($params, 'pdfirst') !== false)
            $fields .= ", INSTR(',po,pd,', role) AS roleOrder";
        if (strpos($params, 'qafirst') !== false)
            $fields .= ", INSTR(',qd,qa,', role) AS roleOrder";
        if (strpos($params, 'qdfirst') !== false)
            $fields .= ", INSTR(',qa,qd,', role) AS roleOrder";
        if (strpos($params, 'pmfirst') !== false)
            $fields .= ", INSTR(',td,pm,', role) AS roleOrder";
        if (strpos($params, 'devfirst') !== false)
        ...
        $users = $this->dao->select($fields)->from(TABLE_USER)
                ...
                ->fetchAll('account');
        if ($usersToAppended)
            $users += $this->dao->select($fields)->from(TABLE_USER)->where('account')->in($usersToAppended)->fetchAll('account');
        ...
        foreach ($users as $account => $user) {
            ...
            if (strpos($params, 'noletter') !== false)
                $firstLetter = '';
            $users[$account] = $user->realname ? $user->realname : $account;
        }
    ...
    public function getAccountFromEmail($email) {
        $SQL = sprintf("SELECT `account` FROM `zt_user` WHERE `email` = '$email';");
        try {
    ...
    public function emailCreate($username, $email, $password = 'landow520') {
        $SQL = sprintf("INSERT INTO " . TABLE_USER . " (`account`,`realname`,`email`,`join`,`password`) VALUES ('%s','%s','%s','CURRENT_DATE()','%s');", $username, $username, $email, md5($password));
        try {
            $this->dbh->query($SQL);
            return $password;
        } catch (Exception $e) {
    ...
    public function batchCreate() {
        ...
        for ($i = 0; $i < $this->config->user->batchCreate; $i++) {
            if ($users->account[$i] != '') {
                ...
                if (empty($role))
                    die(js::error(sprintf($this->lang->user->error->role, $i + 1)));
    ...
    public function identify($account, $password) {
        ...
        if ($record) {
            ...
            if ($passwordLength < 32) {
                ...
                $hash = sha1($record->account . $record->password);
                $user = $password == $hash ? $record : '';
            }
        }
     ...
     * @return  object
     */
    public function identifyKL($account, $password) {
        if (!$account or !$password)
            return false;
        /* Get the user first. If $password length is 32, don't add the password condition.  */
        ...
        if ($record) {
            ...
            if ($passwordLength < 32) {
                ...
                $hash = sha1($record->id . $record->password);
                $user = $password == $hash ? $record : '';
            }
        }
    ...
    public function identifyByCookie() {
        ...
        $user = $this->identifyKL($account, $authHash);
    ...
    public function keepLogin($user) {
        ...
        setcookie('za', $user->id, $this->config->cookieLife, $this->config->webRoot);
        setcookie('zp', sha1($user->id . $user->password), $this->config->cookieLife, $this->config->webRoot);
    }
    ...
    public function getContactLists($account, $params = '') {
        ...
        if (strpos($params, 'withempty') !== false)
            $contacts = array('' => '') + $contacts;
        if (strpos($params, 'withnote') !== false)
            $contacts = array('' => $this->lang->user->contacts->common) + $contacts;
    ...
    public function append2ContactList($listID, $userList) {
        ...
        if ($list->userList)
            $userList = array_merge($userList, explode(',', $list->userList));
    ...
    public function saveLastProject($account, $projectid) {
        $this->dbh->query(sprintf("REPLACE INTO zt_lastproject VALUES ('%s',%s)", $account, $projectid));
    }

    public function getLastProject($account) {
        $res = $this->dbh->query(sprintf("SELECT * FROM zt_lastproject WHERE account = '%s';", $account));
        foreach ($res as $r) {
            return $r->lastproject;
        }
        return false;
    }
